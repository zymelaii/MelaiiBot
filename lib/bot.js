...
class MelaiiBot
{
	...
	constructor(uin, config = null)
	{
		...
		this._max_groupmsg_len = 256; //! 最大群消息长度，超过将截断消息
	...
	get plugins()
	{
		let em = this._em[this._current_env];
		if (em == null) return [];
	...
	registerPlugin(desc)
	{
		if (this._em[this._current_env] == null) return this;
	...
	{
		if (this._em[this._current_env] == null) return -1;
		...
		{
			desc.actions.forEach((e) => {
				let plugin_node = `${desc.plugin}.${e.subame}`;
				let t1 = this.delListener(e.event, `transfer.${plugin_node}`);
				let t2 = this.delListener(`plugin.${plugin_node}`, e.subname);
				if (t1 && t2) ++removed;
			});

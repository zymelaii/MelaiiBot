...
class MelaiiBot
{
	...
	switchTo(env, force = true)
	{
		if (this._current_env == env) return this;

		let last_env = this._current_env;
		if (Object.keys(this._environments).indexOf(env) != -1)
		{	//! 转换环境
		...
		if (last_env != this._current_env)
		{
			let last_listener = this._environments[last_env];
			for (let key in last_listener)
			{
				this._client.off(key, last_listener[key]._);
			}
			for (let key in this.listener)
			{
				this._client.on(key, this.listener[key]._);
			}
		}
	...
	addListener(_event, name, action)
	{
		...
		if (this.listener[_event] == null)
		{
			...
			this.listener[_event]._ = //! dispatcher
			(event) => {
			...
			this._client.on(_event, this.listener[_event]._);
		}
	...
	delListener(event, name)
	{
		...
		if (this.listener[event] != null)
		{
			...
			if (Object.keys(this.listener[event]).length == 1)
			{
				this._client.off(event, this.listener[event]._);

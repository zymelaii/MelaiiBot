...
class MelaiiBot
{
	...
	//!         { env: { event: [{ name, action, priority }, ...], ... }, ... }
	...
	{
		let handlers = [];
		Object.keys(this.listener).forEach((e) => {
			let result = e.match(/^plugin\.([^\.]*?)$/);
			if (!!result)
			{
	...
	emit(signal, data = null)
	{
		...
		// this.trace(`trigger: ${signal} ${data.sub_plugin ?? ''}`);
		return this;
	}
	...
	{
		let domains = _event.split('.');

		//! OICQ 会将事件按域自下而上顺次发送
		//! dispatcher 将只接受原事件并手动分发各级域事件
		//! 后续同一事件的不同域将被忽略
		...
		return async (event) => {
			...
			if (!!this._this_event)
			{
				let keys = Object.keys(event), i = 0;
				for (; i < keys.length; ++i)
				{
					if (event[keys[i]] != this._this_event[keys[i]])
					{
						break;
					}
				}
				if (i == keys.length)
				{
					// this.trace(`@dispatcher: break ${_event}`);
					return;
				}
			}

			this._this_event = clone(event);

			// this.trace(`@dispatcher.${_event}:`);
			// this.trace(`@dispatcher.time=${event.time}`);

			let domain = domains[0], i = 0;
			do {
				let handlers = this.listener[domain];
				for (let j in handlers)
				{
					if (handlers[j].action == null) continue;
					try {
						let shouldContinue = handlers[j].action.call(this, event);
						if (!(shouldContinue ?? true)) return;
					} catch(e) {
						this.error(`failed call action ${handlers[j].name} of ${_event} (${e.message})`);
					}
				}
				domain = `${domain}.${domains[++i]}`;
			} while (i < domains.length);

			// this.trace(`@dispatcher.enclose$`);
		};
	}
	...
	addListener(_event, name, action, priority = 3)
	{
		...
		if (this.listener[_event] == null)
		{
			...
			if (this._dispatchers[_event] == null)
			{
				...
				// this.trace(`addListener ${_event} ${name} 调用次数+1`);
			}
		}

		this.listener[_event].push({ name, action, priority });
		this.listener[_event] = this.listener[_event].sort((a, b) => {
	...
	//! 删除监听器
	//! DONE
	delListener(_event, name)
	{
		let handlers = this.listener[_event];

		if (handlers != null)
		{
			let index = handlers.findIndex((e) => e.name == name);
			if (handlers[index] != null)
			{
				// this.trace(`监听单元注销 ${_event} ${name} - 预备：`, handlers);
				handlers.splice(index, 1);
				// this.trace(`监听单元注销 ${_event} ${name} - 完成：`, handlers);
			}

			if (handlers.length == 0)
			{
				this._client.off(_event, this._dispatchers[_event]);
				delete this._dispatchers[_event];
				delete this.listener[_event];
			}
		}
	...
	registerPlugin(desc)
	{
		...
		desc.actions.forEach((e) => {
			const plugin_name = `${root}.${e.subname}`;
			this.addListener(e.event, `#${plugin_name}`, (event) => {
				let eventData = clone(event);
				eventData.plugin = root;
				eventData.sub_plugin = e.subname;
				this.trace(`trigger: sender=${e.event}.#${plugin_name}; receiver=plugin.${root}`);
				this.emit(`plugin.${root}`, eventData);
			});
			this.addListener(`plugin.${root}`, e.subname, e.action);
		});
	...
	unregisterPlugin(_plugin, force = false)
	{
		...
		{
			desc.actions.forEach((e) => {
				const _event = `plugin.${desc.plugin}`;
				if (this.listener[_event] != null)
				{
					this.delListener(_event, e.subname);
					this.delListener(e.event, `#${desc.plugin}.${e.subname}`);
					++removed;
				}
			});

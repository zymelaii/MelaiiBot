...
{	//! args[index].type == 0
	...
	{
		throw new Error(
			'option "' + option + '" requests '
			+ collect_left + ' more sub-options');
	}
...
function parse(args, description)
{
	...
	args = args.map((e) => {
		...
		return {
			word: ignore.case ? _word.toLowerCase() : _word,
			type: t
		};
	});
	...
	while (i < args.length)
	{
		...
		const option = prefix + args[i].word;
		...
		j = keywords.findIndex((e) => word.word == e.op[key]);
		if (j != -1)
		{
			...
			i += count;
		} else if (!ignore.unknown) throw err_unknown;
		...
		if (freeWords[0].index == 0)
		{
			...
			if (target != null)
			{
				...
				if (tmpargs.length == target.collect)
				{
					...
					{
						throw new Error('subcmd "' + target.keyword + '" requests '
							+ null_count + ' more arguments');
					}
...
function execute(raw_command, description, perform)
{
	...
	return perform(subCmd, argElements, freeWords);
}
